...
const EntryCard = ({ id, title, details, createdAt, done }: Movie) => {
    ...
    return (
        <div className='p-4 border-2 border-palette-3 hover:border-palette-2 duration-150 rounded-xl' style={{ color: !done ? "#483434" : "grey", backgroundColor: !done ? "#EED6C4" : "#ccc2ba" }}>
            <div className='text-xl font-bold m-2 flex justify-between items-center'>
                <p>{dateToShow}</p>
                <div>
                    <button onClick={() => setDoneMutate({ id: id, done: !done })} className='rounded-lg text-sm border-2 p-1 px-2 mr-2 border-palette-2 bg-palette-4 hover:border-palette-4 hover:bg-palette-1 duration-150 hover:text-palette-3'>{done ? "Watched." : "Watched?"}</button>
